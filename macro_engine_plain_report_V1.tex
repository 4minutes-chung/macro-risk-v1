...
\title{Macro Risk Forecasting Package V1: Plain-Language Report}
\author{}
\date{\today}
...
\noindent\textbf{Version note:} This document describes the V1 baseline prototype. It is designed for transparent portfolio documentation and iterative model improvement.
...
\section{Folder map (human-friendly)}
\begin{itemize}
  ...
  \item \texttt{outputs/macro\_engine/}: all generated forecast outputs.
...
\begin{itemize}
  \item Share the \texttt{outputs/macro\_engine/} directory plus \texttt{macro\_engine\_schema.md}.
...
\begin{itemize}
  \item \texttt{outputs/macro\_engine/macro\_forecast\_paths.csv}:
        the full 20-year forecast for every variable and scenario.
  \item \texttt{outputs/macro\_engine/macro\_forecast\_short\_horizon\_intervals.csv}:
        near-term uncertainty bands (5/50/95 percentiles).
  \item \texttt{outputs/macro\_engine/macro\_model\_diagnostics.json}:
        model settings and stability checks.
  \item \texttt{outputs/macro\_engine/macro\_anchor\_assumptions.json}:
        long-run anchors used to guide the forecast.
  \item \texttt{outputs/macro\_engine/macro\_impulse\_responses.csv}:
        impulse responses used for interpretation.
  \item \texttt{outputs/macro\_engine/pd\_macro\_subset\_sample.csv}:
        a small PD-ready file with key macro variables.
  \item \texttt{outputs/macro\_engine/pd\_macro\_subset\_sample.json}:
...
\texttt{outputs/macro\_engine/bvar\_oos\_backtest\_table.csv}.
...
\section{How to run everything (one command)}
...
\section{What the outputs mean (plain language)}
\begin{itemize}
...
\begin{enumerate}
  \item Share the full folder, or at minimum \texttt{outputs/macro\_engine/}.
...
\section{Known limitations (transparent)}
\begin{itemize}
